...
for i in range(1, 100):
    ...
    a.title = '根节点'
    a.node_type = ArticleNodeType.Root.value
    a.root_node = True
    a.no = 'root_%d' % i
    a.save()
...
try:
    ...
    for one in l_one:
        ...
        new_one = Article()
        new_one.s_node = root
        new_one.node_type = ArticleNodeType.Directory.value
        ...
        for two in ArticalLevelTwo.objects.filter(level_one=one):
            ...
            new_two = Article()
            new_two.node_type = ArticleNodeType.Directory.value
            ...
            for atl in Artical.objects.filter(level_two=two):
                ...
                new_atl.node_type = ArticleNodeType.Article.value
